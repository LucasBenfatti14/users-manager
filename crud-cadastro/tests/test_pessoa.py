...
def test_pessoa_normaliza_nome_completo(nome_nao_normalizado):
...
def test_pessoa_aceita_primeiro_nome_valido(primeiro_nome_valido):
...
def test_pessoa_rejeita_primeiro_nome_invalido(primeiro_nome_invalido):
    with pytest.raises(NomeInvalidoError):
        Pessoa(None, primeiro_nome_invalido, 20)

def test_pessoa_aceita_primeiro_nome_acentuado_valido():
...
def test_pessoa_aceita_sobrenome_valido(sobrenome_valido):
...
def test_pessoa_rejeita_sobrenome_invalido(sobrenome_invalido):
    with pytest.raises(NomeInvalidoError):
        Pessoa(None, sobrenome_invalido, 20)

@pytest.mark.parametrize("nome_incompleto", ["Lucas", "Mauro"])
def test_pessoa_rejeita_nome_incompleto(nome_incompleto):
...
def test_pessoa_rejeita_caracteres_invalidos(nome_com_caracteres_invalidos):
